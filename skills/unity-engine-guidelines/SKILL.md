...
description: Behavioral engineering guidelines for writing, editing, and reviewing Unity C# code, to reduce common LLM coding mistakes. Covers surfacing assumptions and tradeoffs before coding, defaulting to garbage-free and SerializeField-wired code (with the full rationale for SerializeField over GetComponent*), making only surgical changes, working toward verifiable goals, respecting the MonoBehaviour lifecycle and Unity 6 stable APIs, never hand-editing serialized asset files outside the Editor, pairing every event subscription with an unsubscription, blending changes into existing code without leaving a trace, exposing only minimal-surface interfaces at system boundaries, and sealing common base-class behavior behind a protected virtual hook instead of a virtual method subclasses can silently break (Template Method over ad-hoc virtual overrides, tied to Liskov substitution). Use whenever writing, editing, or reviewing Unity/C# code in any project.
---
...
## 10. Template Method Over Virtual Base Behavior
*Seal the common flow, expose only the hook.*
- When a base class has behavior that must always run for every subclass (e.g. `BaseWeapon.Fire()`), don't make that method `virtual` and trust subclasses to remember calling `base.Fire()`. Seal it (no `virtual`) and expose a `protected virtual` hook (e.g. `OnFire()`) that subclasses override instead.
- This removes an entire bug class: a subclass override that silently forgets `base.Fire()`, breaking shared behavior (cooldowns, events, VFX, etc.) with no compiler warning.
- If a subclass feels the need to skip or bypass the sealed method's common behavior, that's not a reason to open an escape hatch in the base method, it's a sign the hierarchy violates the Liskov Substitution Principle: that subclass doesn't actually fit the base contract (classic example: `Bird.Fly()` with a `Penguin` that can't fly). Fix the hierarchy instead, e.g. extract a narrower interface for the capability not every subclass shares, rather than letting a subtype opt out of the base contract.
